...
class D1AddSectionModal {
     ...
     */
    renderContent() {
        const existingSections = window.d1State.get('d2Sections', []).map(s => s.id);
    ...
    addSection(sectionType) {
        const newSection = window.d1State.addSection(sectionType);
        if (newSection) {
            // Seleciona a nova seção
            window.d1State.selectSection(newSection.id);
...
// Exporta globalmente
window.D1AddSectionModal = D1AddSectionModal;
